...
above the adapted block in the source file. Required code attribution lives in this repo rather than UI
chrome; the Map legend may identify its factual geographic sources so players can distinguish base data from
project-authored lore and overlays.
...
- **MapLibre GL JS 5.24.0** — vendored browser distribution from the
  [MapLibre GL JS project](https://github.com/maplibre/maplibre-gl-js), licensed under BSD-3-Clause with
  bundled component notices. The retained licence, source record, and immutable hashes live in
  [`vendor/maplibre/`](vendor/maplibre/). Project-specific layers and integration live in `map-terrain.js`.

## Geographic data

- **USGS 3D Elevation Program (3DEP)** — public-domain elevation products from the
  [USGS National Map](https://www.usgs.gov/3d-elevation-program). The bounded relief tiles are an original,
  reproducible presentation generated by `tools/build-map-terrain.py`; the service snapshot, scope, palette,
  counts, and hashes are recorded in `media/map-terrain/manifest.json`.
- **Natural Earth 1:110m physical data** — public-domain land, lakes, and major-river geometry from
  [Natural Earth](https://www.naturalearthdata.com/), pinned to the source commit and hashes recorded in
  [`data/geography/README.md`](data/geography/README.md). Credit is optional under Natural Earth's terms;
  it is retained here for geographic provenance.
