...
| **Fallouty** | `Fallouty.woff2` | Sébastien Caisse ("Red!"), 2002 — the flagship/iconic tribe face per `TASKS.md` T38 | Custom licence **embedded in the font file itself** (no separate `fonts/Fallouty-LICENSE.txt` exists yet) | ⚠ Needs attribution — action required | The embedded licence *does* grant free use, but with real conditions: (1) keep the licence/copyright notice with any redistributed copy, (2) forward modifications back to the author if you make any, and — the one not yet met — **(3) for web-enabled media, credit the author somewhere the app shows credits, AND provide a valid link to the original font indicating its version and copyright owner.** `CREDITS.md` currently has no Fallouty entry at all. | Add a `CREDITS.md` line (template below) with the embedded copyright notice + a link to the original font source, and extract the licence text out of the font's `name` table into `fonts/Fallouty-LICENSE.txt` so it's readable without a font tool (same pattern as every other font here). This is a quick, concrete fix — not a reason to drop the font. |
| *(unused reference files)* | `Dymo-README.txt`, `EmbossingTape-LICENSE.txt`, `fzmEmbossed-LICENSE.txt`, `MyUnderwood-LICENSE.txt`, `VeteranTypewriter-LICENSE.txt` | various | n/a — no matching font files exist in `fonts/`, and nothing in `styles.css` references them | ✅ Safe / not applicable | These match the **CSS techniques** already logged in `CREDITS.md`'s "Technique inspiration" section (the Dymo/embossed-label-tape effect) — studied for the *look*, then reimplemented from scratch in CSS, not embedded as actual fonts. Not a licensing exposure since nothing is redistributed. | No action required; optionally delete them or add one line to `CREDITS.md` noting they're kept as provenance for the embossed-label CSS effect, so a future sweep doesn't have to re-derive that they're unused. |
...
(Overseer/Fallouty terms are quoted directly from `fonts/Overseer-LICENSE.txt` and the `Fallouty.woff2` embedded `name` table — read via `fonttools`.)
...
- Attribute game-icons.net icons per-author as you adapt them (T88) — do it as you go, not as a batch
  cleanup later.
- Add the Fallouty + OFL-font credit lines to `CREDITS.md` (above) — quick, and closes the one real
  outstanding licence obligation found in this sweep.
